...
try {
  ...
  assert(rails.game.width > 150 && rails.leftRight <= rails.game.x && rails.game.right <= rails.rightX,
    'The game must be between the two independent controller rails');
  ...
  console.log('Landscape handheld, Godot iframe, and simultaneous joystick/attack browser checks passed.');
} finally {
  await browser.close();
}
